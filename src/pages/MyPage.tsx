...
const dummyAnalyses: Analysis[] = [
  ...
  },
  {
    ...
    date: "Jul 10, 2022",
    total: "$38.00",
    images: ["/assets/img/account/orders/01.png", "/assets/img/account/orders/05.png"],
  },
  {
    id: "#502TR872W2",
...
export const MyPage: FC<MyPageProps> = ({
  ...
  const isFormValid = nicknameInput.trim().length > 0 && isPhoneValid && isPasswordValid;
        ...
        return (
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <form onSubmit={handleSave} className="space-y-5 max-w-2xl">
              <div>
                <label className="block text-sm text-gray-500 mb-1">이름 (읽기 전용)</label>
                <input
                  type="text"
                  value={profile.name}
                  disabled
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-500 mb-1">성별 (읽기 전용)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm text-gray-500 mb-1">생년월일 (읽기 전용)</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {passwordInput !== "" && passwordInput.length < 8 && (
                    <p className="mt-1 text-xs text-red-600">비밀번호는 8자 이상이어야 합니다.</p>
                  )}
                </div>
              </div>
      ...
      case "analysis":
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl p-6 space-y-4 shadow-sm">
              {(() => {
                ...
                return ordered.map((a, idx) => {
                  const isInProgressOpen = a.status === "inProgress" && openInProgressId === a.id;
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-6">
                        <span className="text-sm text-gray-500 w-40 truncate">{a.id}</span>
                        <span
                          className={`
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="relative ml-4">
                          <button
                            ...
                            onClick={() => {
                              if (a.status === "completed") {
                                ...
                                setOpenInProgressId((prev) => (prev === a.id ? null : a.id));
                              } else if (a.status === "failed") {
                                setOpenFailed({
                                  id: a.id,
                          ...
                          >
                            {a.status === "inProgress" ? (isInProgressOpen ? "v" : ">") : ">"}
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            {openFailed && (
              <div className="fixed inset-0 z-40 flex items-center justify-center">
                <div className="absolute inset-0 bg-black/40" onClick={() => setOpenFailed(null)} />
                <div className="relative z-50 w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-semibold text-red-600">분석 실패</h3>
                    <button
                      type="button"
                      className="p-1 rounded hover:bg-gray-100"
